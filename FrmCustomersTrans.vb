Public Class FrmCustomersTrans
    Sub fillDataGriedView(sql)
        'ط========عرض سندات استلام الاصناف من المورد
        DataGridView1.Rows.Clear()
        '=========================== الكمية التى تم توريدها الى المخزن
        Dim str = " select  sum(VoucherDebit) , sum(VoucherCredit) from CustomerTrans where  CustomerName=N'" & CustomerName.Text & "' and VoucherDate<'" & Format(DateFrom.Value, "yyy/MM/dd") & "'"
        Dim cmd As New SqlClient.SqlCommand(str, SQLconn)
        Dim da As SqlClient.SqlDataAdapter = New SqlClient.SqlDataAdapter(cmd)
        Dim dta As New DataTable("CustomerTrans")
        da.Fill(dta)
    ...
    Function GetSupplierCODe(SupName) As String
        Dim Adp = New SqlClient.SqlDataAdapter("select * from Customers where CustomerName=N'" & SupName & "' ", SQLconn)
        Dim DS = New DataSet
        Adp.Fill(DS)
        Dim dt As New DataTable
        dt = DS.Tables(0)
        If dt.Rows.Count > 0 Then Return dt.Rows(0).Item("CustomerCode") Else Return ""
    End Function
    Sub FillSupplierCombo()

        CustomerName.Items.Clear()
        Dim sql = "select * from Customers order by CustomerName"
        Dim Adp = New SqlClient.SqlDataAdapter(sql, SQLconn)
        Dim DS = New DataSet
        Adp.Fill(DS)
        Dim dt As New DataTable
        ...
        For I = 0 To dt.Rows.Count - 1
            ...
            CustomerName.Items.Add(dt.Rows(I).Item("CustomerName"))
            ', dt.Rows(I).Item("SuPInternal_ID"), dt.Rows(I).Item("SupplierCode")
        Next
    End Sub
    Private Sub FrmCustomersTrans_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        CLR()
    End Sub
    ...
    Private Sub CustomerName_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles CustomerName.SelectedIndexChanged
        CustomerCode.Text = GetSupplierCODe(CustomerName.Text)
    End Sub

    Private Sub BtnView_Click(sender As System.Object, e As System.EventArgs) Handles BtnView.Click
        fillDataGriedView("select * from CustomerTrans  where  CustomerName=N'" & CustomerName.Text & "' and VoucherDate>='" & Format(DateFrom.Value, "yyy/MM/dd") & "' and VoucherDate<='" & Format(DateTo.Value, "yyyy/MM/dd") & "' order by VoucherDate ")

    End Sub

    Private Sub FrmCustomersTrans_KeyDown(sender As System.Object, e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
